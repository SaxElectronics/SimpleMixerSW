...
#include <AudioEffects/Compressor/Compressor.h>
#include <AudioEffects/Filters/Filters.h>
#include <AudioEffects/Filters/Filters.h>
#include <AudioEffects/PitchShift/PitchShift.h>
#include <AudioEffects/Reverb/Reverb.h>
#include <stdint.h>
#include <math.h>
#include "Audio.h"
...
static __attribute__((__noreturn__)) void OsTask_AudioProcess(void *arg)
{
    ...
    while (1)
    {
        ...
        for (int i=0+BufferOffset; i<64+BufferOffset; i=i+2) {
            ...
            lSample = (float) DoCompressor(lSample);
            lSample = Do_Reverb(lSample);
            rSample = (float) Calc_FIR(rSample);
            //run HP on left channel and LP on right channel
            lSample = (float) Calc_IIR(lSample,&IIR_ParametersLeft, &IIR_DataLeft);
            rSample = (float) Calc_IIR(lSample,&IIR_ParametersRight, &IIR_DataRight);
